...
async function getSigner(chain, gasLimit) {
    ...
    switch (platform) {
        case "Solana":
            signer = await (await (0, solana_1.default)()).getSigner(await chain.getRpc(), env_1.ENV.SOLANA_ACCOUNT_PRIVATE_KEY);
            break;
        case "Evm": {
            const evmSignerOptions = gasLimit ? { gasLimit } : {};
            signer = await (await (0, evm_1.default)()).getSigner(await chain.getRpc(), env_1.ENV.EVM_ACCOUNT_PRIVATE_KEY, evmSignerOptions);
            break;
        }
        default:
...
const createWrappedToken = async (destinationChain, tokenAddress, network) => {
    try {
